...
def create_app():
    ...
    def create_user(current_user):
        ...
        if(validate_email(data['name']) == False):
            return jsonify({'message': 'You must enter a valid email address'})
    ...
    def delete_specific(current_user):  
        ...
        try:
            ...
            if len(ids) == 0:
                response = {'result': 'No document found with the provided name'}
            else:
    ...
    @application.route("/api/getsources", methods=["GET"])
    @token_required
    def getsources(current_user):
        if not current_user.admin:
            return jsonify({'message': 'You must be an admin to delete a specific document from the database'})

        try:

            sources = list(get_all_sources())
